...
if ($_GET['insert']) {
  ...
  if ($_GET['type'] == "image") {
    ...
    media_send_to_editor("[pressbox path=" . $_GET['path'] . " type=x name=".$_GET['name']."]");
  } else {
  ...
  $share = lh_pressbox_get_shares($connection, $_GET['path']);
} else {
  $folders = array();
  $files = array();
        ...
        <div id="pressbox-display-box">
          ...
          <div id="pressbox-metadata">
            <ul>
			  ...
			  <li>SharedPath: <?php echo "<a href='./media-upload.php?post_id=".$_GET['post_id']."&type=x&tab=pressbox&path=".$share->{'url'}."&choose=1&insert=1&name=".basename($metadata->{'path'})."' >".$share->{'url'}."</a>"; ?></li>
            </ul>
          </div>
          </div>
