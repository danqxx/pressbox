...
if (isset($_GET['uid'])) {
  ...
  //echo("getAccessToken(".get_option('lh_pressbox_consumer_key').", ".get_option('lh_pressbox_consumer_secret').", ".get_option('lh_pressbox_oauth_token').", ".get_option('lh_pressbox_oauth_token_secret'));
  //echo get_option('lh_pressbox_oauth_token_secret');
  ...
  if ($connection->http_code == "200") { // We got our token.
    ...
    $error = "Pressbox could not connect to your DropBox Account. Verify your Key and Secret and try again. {$connection->http_code} & {$connection->decode_json}";
  }
